...
function generateSpeakers() {
    var speakersConainer =  document.querySelector(".speakers-grid");
    console.log("speakersConainer", speakersConainer);
    for (var i = 1; i <= 6; i++) {
        var rndInt = randomIntFromInterval(0, 11);
        while (chosenSpeaker.includes(rndInt)) {
            rndInt =  randomIntFromInterval(0, 11);
        }
        chosenSpeaker.push(rndInt);
        var speaker = speakers[rndInt];
